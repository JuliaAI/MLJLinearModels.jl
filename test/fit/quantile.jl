...
if DO_COMPARISONS
    r_results = TOML.parsefile(joinpath(@__DIR__, "quantile.toml"))
end
...
@testset "QuantileReg" begin
    ...
    if DO_COMPARISONS
        ...
        θ_lbfgs  = fit(rr, X, y1a, solver=LBFGS())
        θ_qr_br2  = r_results["θ_qr_br2"]

        @test isapprox(J(θ_qr_br2), 404.6161, rtol=1e-4)
