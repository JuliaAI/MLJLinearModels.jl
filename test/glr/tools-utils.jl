...
@testset "Objectives" begin
    ...
    J = objective(r, X, y1)
    @test J(θ1) ≈ sum(abs2.(X_*θ1 - y1))/2 + λ * sum(abs2.(θ1))/2
    r = RidgeRegression(λ; fit_intercept=true, penalize_intercept=false)
    J = objective(r, X, y1)
    @test J(θ1) ≈ sum(abs2.(X_*θ1 - y1))/2 + λ * sum(abs2.(θ1 .* vcat(ones(p), 0)))/2

    # L2 / L2+L1
    r = ElasticNetRegression(λ, γ; fit_intercept=true, penalize_intercept=true)
    J = objective(r, X, y1)
    @test J(θ1) ≈ sum(abs2.(X_*θ1 - y1))/2 +
                    λ * sum(abs2.(θ1))/2 +
                    γ * sum(abs.(θ1))
    ...
    J = objective(r, X, y1)
    @test J(θ1) ≈ LogisticLoss()(y1, X_*θ1) +
                    λ * sum(abs2.(θ1))/2 +
                    γ * sum(abs.(θ1))
    ...
    J = objective(r, X, y1)
    @test J(θ1) ≈ RobustLoss(HuberRho(δ))(y1, X_*θ1) +
                    λ * sum(abs2.(θ1))/2 +
                    γ * sum(abs.(θ1))
